...
or the control volume difference method [5]. The control volume method will be
used  since it is the same method utilized in the original version of CTF. The
implicit  finite difference equation now looks like equation \ref{eq:}

\begin{equation}
	\label{eq:rod:fuel:conduction}
	  \rho_{i}c_{p,i}\frac{T^{n+1}_{i}-T^{n}_{i}}{\Delta t}
	- \frac{2\pi}{A_{i}}\\\left( 
		\left(kr\frac{\partial T}{\partial r}\right)_{i+\frac{1{2}}
		\left(kr\frac{\partial T}{\partial r}\right)_i-\frac{1{2}}
		\right)
	- q'''
	= 0
\end{equation}
